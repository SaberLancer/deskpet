...
from web import create_chat_header, create_request_header
...
class SparkWeb:
    ...
    def __init__(self, cookie, fd, GtToken, ChatID=""):
        ...
        self.__chat_header = create_chat_header(cookie)
    ...
    def __generate_chat_id(self):
        if self.__chat_id == "":
            ...
            response = requests.request(
                "POST", url, headers=self.__request_header, data=payload)
    ...
    def __set_name(self, chat_name):
        ...
        response = requests.request(
            "POST", url, headers=self.__request_header, data=json.dumps(payload))
        response_data = json.loads(response.text)
        if response_data['code'] != 0:
            print(
    ...
    def __get_resp(self, question):
        ...
        response = requests.request(
            "POST", url, headers=self.__chat_header, data=payload, stream=True)
        return response
    ...
    def chat_stream(self, history: bool = False, history_path: str = './history/'):
        ...
        if history:
            ...
            history_file_path = os.path.join(
                history_path, f'history_{str(datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))}.json')
        try:
            self.__create_chat(NEW_CHAT)
